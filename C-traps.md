...
## 三目操作符⚠️
在C语言中，**三目操作符（条件操作符 `?:`）的语法要求两个表达式都必须存在**，但第二个和第三个操作数必须是表达式，不能省略
...
i+1;//i = 0;
//如指针的引用
arr[i+1];//i仍未0，只读而不会改变

//若要使i增加需有赋值动作
i = i + 1;//i = 1
i += 1;//i = 2
```
❗❗❗可以看出`++`既有运算又有重新赋值动作，而单独的`+1`只读取当前运算结果不会对变量重新赋值

---

## `free`释放内存
⚠️⚠️⚠️`free`的参数必须对应动态开辟内存时返回的指针，当动态开辟内存后返回的指针被修改时，`free`无法释放其指向的内存空间
![](./Image/C-traps-1774705731978.webp)
